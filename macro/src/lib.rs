...
fn get_key_code_winit(tree: TokenTree) -> Option<Ident> {
    match tree {
...
fn get_key_code_bevy(tree: TokenTree) -> Option<Ident> {
    match tree {
        TokenTree::Literal(ref literal) => {
            let x = literal.span().source_text().unwrap();
            if x.len() == 1 && x.parse::<u8>().is_ok() {
...
fn partial_key(input: TokenStream) -> (TokenStream, TokenStream) {
    ...
    while let Some(tree) = i.next() {
        if i.peek().is_none() || (!is_dash(&tree) && !is_dash(i.peek().unwrap())) {
            if cfg!(feature = "bevy") {
                key_code = get_key_code_bevy(tree);
            } else if cfg!(feature = "winit") {
                key_code = get_key_code_winit(tree);
            } else {
                key_code = get_key_code_bevy(tree);
            }
            break;
